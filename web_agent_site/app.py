...
from web_agent_site.engine.engine_llm import (
    load_products,
    init_search_engine,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_llm", action='store_true', help="Use LLM for search ranking")
    ...
    # Check if Ollama server is running if LLM is enabled
    if USE_LLM:
        import subprocess
        try:
            print("Checking Ollama CLI...")
            result = subprocess.run(
                ["ollama", "run", "mistral:latest", "Hi"],
                capture_output=True,
                text=True,
                timeout=50
            )
            ...
            if result.returncode == 0:
                print("Ollama CLI is working")
            else:
                print(f"Ollama CLI error: {result.stderr}")
                print("Continuing without LLM ranking")
                USE_LLM = False
