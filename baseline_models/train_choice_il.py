...
from models.sbert import SBertConfigForWebshop, SBertModelForWebshop
from models.t5 import T5ConfigForWebshop, T5ModelForWebshop
...
def get_dataset(split, mem=False, tokenizer=None):
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="./ckpts/web_click/e5",
...
def main():
    ...
    wandb.init(project="sbert_il", config=args, name=args.output_dir)
    ...
    if model_type == "bert-base-uncased":
    ...
    elif model_type == "microsoft/mpnet-base":
        config =  BertConfigForWebshop(
            pretrained_model=args.pretrain,
            image=args.image,
            model_name=args.encoder_model
        ...
        model =  BertConfigForWebshop(config)

    elif model_type == "sentence-transformers/all-MiniLM-L6-v2":
        config = SBertConfigForWebshop(
            pretrained_model=args.pretrain,
            image=args.image,
            model_name=args.encoder_model
        )
        
        model = SBertModelForWebshop(config)

    elif model_type == "t5":
        config = T5ConfigForWebshop(
            pretrained_model=args.pretrain,
            image=args.image,
            model_name=args.encoder_model
        )
        
        model = T5ModelForWebshop(config)
    ...
    # Resize token embeddings dynamically
    model.resize_token_embeddings(len(tokenizer))
    # model.bert.resize_token_embeddings(len(tokenizer))

    train_dataset = get_dataset("train", mem=args.mem, tokenizer=tokenizer)
    eval_dataset = get_dataset("eval", mem=args.mem, tokenizer=tokenizer)
